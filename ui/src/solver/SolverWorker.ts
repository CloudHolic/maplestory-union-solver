...
export class SolverWorker {
     ...
     */
    solve(input: ExactCoverInput, options: SolveOptions): Promise<ExactCoverResult> {
        if (this.started) {
            return Promise.reject(new Error('SolverWorker is one-shot; create a new instance.'));
        }
        this.started = true;

        const { promise, resolve, reject } = Promise.withResolvers<ExactCoverResult>();
