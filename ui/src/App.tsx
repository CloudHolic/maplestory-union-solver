...
function App() {
  const [inputJson, setInputJson] = useState(() => JSON.stringify(SAMPLE_INPUT, null, 2))
  const [optionsJson, setOptionsJson] = useState(() => JSON.stringify(SAMPLE_OPTIONS, null, 2))
  ...
  return (
      <div className="min-h-screen bg-background text-foreground p-6">
        ...
        <p className="text-sm text-foreground/60 mb-6">
          Slice 1 verification — paste an ExactCoverInput and SolveOptions, run the
          solver via Worker, see the result. Cancel mid-solve to verify cooperative
          cancellation produces partial stats.
        </p>

        <div className="grid grid-cols-2 gap-4 mb-4">
          <label className="flex flex-col">
            <span className="text-sm font-semibold mb-1">ExactCoverInput (JSON)</span>
            <textarea
                value={inputJson}
          ...
          <label className="flex flex-col">
            <span className="text-sm font-semibold mb-1">SolveOptions (JSON)</span>
            <textarea
                value={optionsJson}
...
function ResultPanel({ run }: { run: RunState }) {
  switch (run.kind) {
    case 'idle':
      return <p className="text-foreground/60">Idle. Click Solve to start.</p>
      ...
      return (
          <div>
            <p className="font-semibold">
              Result: {status} — {elapsedMs.toFixed(1)}ms wall, {result.stats.elapsedMs}ms solver
            </p>
