...
| Path | License | File |
|------|---------|------|
| `solver/**` | AGPL-3.0-or-later | [`solver/LICENSE`](solver/LICENSE) |
| `poset/**` | GPL-3.0-or-later | [`poset/COPYING`](poset/COPYING) |
| `backend/**` | MIT | [`backend/LICENSE`](server/LICENSE) |
...
- **`solver/` (AGPL-3.0-or-later)**: The solver is the substantive
...
- **`poset/` (GPL-3.0-or-later)**: The training pipeline is a
...
- **`backend/` (MIT)**: The server is a thin proxy/cache layer
...
- **`frontend/` (MIT)**: Frontend code is conventional and has lower reuse
...
`solver/` and `poset/` do not share source code in either direction
